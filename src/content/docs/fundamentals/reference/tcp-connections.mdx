...
An origin connection can remain idle for up to 30 seconds. After 30 seconds, a keepalive is issued. If an origin does not respond to the first keepalive after 5 seconds, another keepalive is issued. If the origin does not respond for 5 seconds after this second keepalive, Cloudflare will sever the connection by sending a TCP Reset (RST) packet.[^1]

[^1] https://github.com/cloudflare/pingora/blob/21703dfe835f7a8301fd60ccf0a0d61ada88fb72/pingora-core/src/protocols/l4/ext.rs#L566-L580
